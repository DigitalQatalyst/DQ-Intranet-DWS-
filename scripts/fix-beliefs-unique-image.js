...
async function fixBeliefsUniqueImage() {
  console.log('🖼️  Assigning unique image to DQ Beliefs...\n');
  ...
  usedPhotoIds.forEach(id => {
    const guide = allGuides.find(g => {
      const match = g.hero_image_url?.match(/photo-([a-z0-9-]+)/);
      return match && match[1] === id;
    });
    console.log(`   - ${id} (${guide?.title || 'unknown'})`);
  });

  // Available photo IDs for beliefs (values, principles, team collaboration)
  const availablePhotoIds = [
    ...
    '1507003211169-0a1dd7228fbb'  // Skills/capabilities
  ];

  // Find first available photo ID that's not used
  ...
  if (beliefsError) {
    console.error('❌ Error:', beliefsError);
    return;
  }

  // Create unique image URL
  const hash = createHash('md5').update(`${beliefs.id}-DQ Beliefs-unique`).digest('hex');
  const uniqueParam = hash.substring(0, 8);
  const newImageUrl = `https://images.unsplash.com/photo-${selectedPhotoId}?w=800&h=400&fit=crop&q=80&u=${uniqueParam}`;

  console.log(`\nUpdating "DQ Beliefs"...`);
  console.log(`   Current: ${beliefs.hero_image_url?.substring(0, 70)}...`);
  console.log(`   New Photo ID: ${selectedPhotoId}`);
  console.log(`   New URL: ${newImageUrl.substring(0, 70)}...`);
  console.log(`   Theme: Values, principles, team collaboration (unique from Competencies)`);
  ...
  if (allUnique) {
    ...
    console.log(`   DQ Competencies: Kept original image (1521737854947-0b219b6c2c94)`);
    console.log(`   DQ Beliefs: New unique image (${selectedPhotoId})`);
  } else {
    console.log(`\n⚠️  Some duplicates found.`);
  }
}

fixBeliefsUniqueImage().catch(console.error);
