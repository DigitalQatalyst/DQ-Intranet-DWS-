...
async function cleanBlueprintStructure() {
  console.log('🔄 Cleaning Blueprint Structure section to show only Feature Modules...\n')
  ...
  // Find the Blueprint Structure section and replace it with clean version
  const blueprintStructureRegex = /## Blueprint Structure[\s\S]*?(?=### AI Tools|##)/
  
  const cleanBlueprintStructureSection = `## Blueprint Structure
...
`
  ...
  // Replace the entire Blueprint Structure section
  newBody = newBody.replace(blueprintStructureRegex, cleanBlueprintStructureSection)
  ...
  if (data) {
    console.log('✅ Successfully cleaned Blueprint Structure section!')
    console.log(`   Title: ${data.title}`)
    console.log(`   Slug: ${data.slug}`)
    console.log('\n✅ Blueprint Structure now shows only Feature Modules listing - no other notes or explanations.')
  }
}

cleanBlueprintStructure().catch(console.error)
