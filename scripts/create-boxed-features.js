...
async function createBoxedFeatures() {
  console.log('Creating boxed Features section...\n');
  ...
  try {
    ...
    const newBody = beforeFeatures + boxedFeatures + '\n\n' + afterFeatures;
    ...
    if (data && data.length > 0) {
      console.log('✓ Features section created with boxed format!');
    } else {
      console.log('⚠ No blueprint found with slug "dws-blueprint"');
    }
  } catch (error) {
    console.error('✗ Error creating boxed Features:', error.message);
  }
