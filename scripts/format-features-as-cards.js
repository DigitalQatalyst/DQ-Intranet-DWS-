...
const cardFormattedFeatures = `## Features
...
async function formatFeaturesAsCards() {
  console.log('Formatting Features section as cards...\n');
  ...
  try {
    ...
    const newBody = beforeFeatures + cardFormattedFeatures + '\n\n' + afterFeatures;
    ...
    if (data && data.length > 0) {
      console.log('✓ Features section formatted as cards!');
      console.log('✓ Each feature now has:');
      console.log('  - Title (H3 heading)');
      console.log('  - Description paragraph');
      console.log('  - Bulleted list of components and considerations');
    } else {
      console.log('⚠ No blueprint found with slug "dws-blueprint"');
    }
  } catch (error) {
    console.error('✗ Error formatting Features:', error.message);
  }

  console.log('\nDone!');
}

formatFeaturesAsCards();
