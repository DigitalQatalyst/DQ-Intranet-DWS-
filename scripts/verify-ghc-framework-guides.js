...
async function verifyGHCFrameworkGuides() {
  console.log('🔍 Verifying GHC Framework guides...\n');
  ...
  // Filter for GHC framework guides
  const frameworkGuides = (allGuides || []).filter(g => {
    const subDomain = (g.sub_domain || '').toLowerCase();
    const domain = (g.domain || '').toLowerCase();
    const title = (g.title || '').toLowerCase();
    return subDomain.includes('ghc') && 
           !subDomain.includes('ghc-leader') &&
           domain === 'strategy';
  });

  console.log(`📊 Found ${frameworkGuides.length} GHC Framework guides:\n`);
...
verifyGHCFrameworkGuides().catch(console.error);
