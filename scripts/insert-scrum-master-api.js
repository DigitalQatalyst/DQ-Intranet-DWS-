// scripts/insert-scrum-master-api.js
// Insert the Scrum Master Framework guide using the admin API
// Usage: node scripts/insert-scrum-master-api.js
...
async function main() {
  console.log('Inserting Scrum Master Framework guide via Supabase...')
  
  // Try to use Supabase directly
  ...
  if (existing) {
    ...
    console.log(`✓ Guide is now available at: /marketplace/guides/${guideData.slug}`)
  } else {
    const { data, error } = await sb
      .from('guides')
      .insert(guideData)
  ...
  console.log('Done! The guide should now appear in the Guidelines section of the frontend.')
}
