...
async function regenerateAIToolsClean() {
  console.log('🔄 Deleting entire AI Tools section and regenerating with only Cursor and Windsurf...\n')
  ...
  // Create clean AI Tools section with only Cursor and Windsurf
  const cleanAITools = `### AI Tools
...
`
  ...
  // Find and replace the entire AI Tools section
  const aiToolsRegex = /### AI Tools[\s\S]*?(?=### Model Provider|##)/
  
  if (aiToolsRegex.test(newBody)) {
    newBody = newBody.replace(aiToolsRegex, cleanAITools.trim() + '\n\n')
  } else {
  ...
  if (data) {
    console.log('✅ Successfully regenerated AI Tools section!')
    console.log(`   Title: ${data.title}`)
    console.log(`   Slug: ${data.slug}`)
    console.log('\n✅ AI Tools section now contains only Cursor and Windsurf (no duplicates).')
  }
}

regenerateAIToolsClean().catch(console.error)
