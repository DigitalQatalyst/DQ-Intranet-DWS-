...
import { CARD_ICON_BY_ID, DEFAULT_COURSE_ICON } from "../../utils/lmsIcons";
import { SFIA_LEVELS } from "@/lms/config";
import { formatDurationFromMinutes } from "../../utils/durationFormatter";
import {
  findLearningPathsForCourse,
  fetchCoursesInLearningPath,
...
type FlatLesson = { id: string; order: number; type: string };

function flattenItemLessons(item: any, lessons: FlatLesson[]): void {
  if (item.topics?.length) {
    ...
    const sortedLessons = [...item.lessons].sort(
      (a: any, b: any) => a.order - b.order,
    );
    sortedLessons.forEach((l: any) => {
      lessons.push({ id: l.id, order: lessons.length, type: l.type });
...
export const LmsCourseDetailPage: React.FC = () => {
  ...
  const [hasInitializedCurriculum, setHasInitializedCurriculum] =
    useState(false);
  ...
  // Save for Later
  const { isSaved, toggle: toggleSave } = useSaveForLater(
    course?.id ?? "",
    course?.slug ?? "",
  );
  ...
  const { data: learningPaths = [] } = useQuery({
    ...
    queryFn: () =>
      course?.id ? findLearningPathsForCourse(course.id) : Promise.resolve([]),
    enabled: !!course?.id,
  });
  ...
  const { data: pathCourses = [] } = useQuery({
    ...
    queryFn: () =>
      firstPath?.pathId
        ? fetchCoursesInLearningPath(firstPath.pathId)
        : Promise.resolve([]),
  ...
  React.useEffect(() => {
    if (course) {
      ...
      console.log(
        "[LMS Detail Page] Full course object:",
  ...
  const allFlattenedLessons = useMemo(() => {
    ...
    const sorted = [...curriculum].sort((a, b) => a.order - b.order);
    sorted.forEach((item) => {
      flattenItemLessons(item, lessons);
    });
    return lessons;
  }, [curriculum]);
  ...
  const relatedCourses = useMemo(() => {
    ...
    return allCourses.filter(
      (detail) =>
        detail.courseCategory === course.courseCategory &&
        detail.id !== course.id,
    );
  }, [course, allCourses]);
        ...
        <div className="flex-grow flex items-center justify-center px-4">
          <div className="text-center max-w-md">
            ...
            <p className="text-gray-600 mb-6">
              {courseError.message ||
                "An error occurred while loading the course details."}
            </p>
            <button
  ...
  if (
    !course ||
    (course.slug !== slug && course.slug.toLowerCase() !== slug?.toLowerCase())
  ) {
    return (
      <div className="min-h-screen flex flex-col bg-gray-50">
        <Header
        ...
        <div className="flex-grow flex items-center justify-center px-4">
          <div className="text-center max-w-md">
            ...
            <p className="text-gray-600 mb-6">
              We couldn't locate that course or track. Head back to the learning
              center to explore the latest learning paths.
            </p>
            <button
              onClick={() => navigate("/lms")}
              className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors"
      ...
      ? [
          {
            ...
            label: "FAQ",
            description:
              "Find answers to the most frequently asked questions about this learning track. Topics covered include prerequisites, how to access the content, expected time commitment, certification details, and what support is available to you throughout your learning journey.",
          },
        ]
      : []),
  ];
  ...
  const sidebarRows = [
    ...
    },
    {
      label: "Level",
      value:
        SFIA_LEVELS.find((level) => level.code === course.levelCode)?.label ||
        course.levelCode ||
        "N/A",
    },
    ...(courseStats.totalModules > 0
      ? [
          {
            label: "Modules",
            value: `${courseStats.totalModules} ${courseStats.totalModules === 1 ? "module" : "modules"}`,
          },
        ]
      : []),
  ];
          ...
          <div className="lg:col-span-2 space-y-6">
                ...
                {firstPath && pathCourses.length > 0 && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
                    ...
                    <p className="text-gray-700 mb-4 text-sm">
                      This course is part of a larger learning track. Explore
                      other courses in this track to complete your learning
                      journey.
                    </p>
                    <div className="flex flex-wrap items-center gap-2">
                      {pathCourses.map((pathCourse, index) => {
                        const isCurrentCourse =
                          pathCourse.slug === course?.slug;
                        return (
                  ...
                  <div className="space-y-4">
                    {curriculum
                      ...
                      .map((item, curriculumIndex) => {
                        ...
                        const isFinalAssessmentModule = moduleLessons.some(
                          (l) => l.type === "final-assessment",
                        );

                        return (
                          <div
                            key={item.id}
                            className={`bg-white border border-gray-200 rounded-lg overflow-hidden transition-all ${isFinalAssessmentModule ? "mt-8" : ""}`}
                          >
                            {/* Module Header */}
                            <div
                            ...
                            >
                              <div className="flex items-center gap-4">
                                <div
                                  className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${isFinalAssessmentModule ? "bg-blue-100 text-blue-600" : "bg-blue-50 text-blue-600"}`}
                                >
                                  ...
                                  return (
                                    <div
                                      ...
                                      onClick={() =>
                                        !isLocked &&
                                        navigate(
                                          `/lms/${course.slug}/lesson/${lesson.id}`,
                                        )
                                      }
                                    ...
                                    >
                                      ...
                                      <div className="flex-1 min-w-0">
                                        ...
                                        <h4
                                          className={`text-base font-semibold mb-1 ${isLocked ? "text-gray-500" : "text-gray-900"}`}
                                        >
                                          {lesson.title}
                                        </h4>
                                        {lesson.description && (
            ...
            {course.track && !isTrack && (
              <section className="mt-8 p-6 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <p className="text-gray-700 mb-4">
                  This course is part of a larger learning track. Explore other
                  courses in this track to complete your learning journey.
                </p>
            ...
            {isTrack && course.curriculum && course.curriculum.length > 0 && (
              <section className="mt-8 p-6 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <p className="text-gray-700 mb-4">
                  This track contains {curriculum.length}{" "}
                  {curriculum.length === 1 ? "course" : "courses"}. Complete all
                  courses to master the full learning journey. Each course can
                  be accessed individually, and you can view the detailed
                  curriculum in the Curriculum tab above.
                </p>
                ...
                {curriculum.length > 5 && (
                  <p className="text-sm text-gray-600 mt-2">
                    and {curriculum.length - 5} more{" "}
                    {curriculum.length - 5 === 1 ? "course" : "courses"}
                  </p>
                )}
              </section>
            )}
        ...
        >
          ...
          >
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <div>
                <h2 className="text-lg font-semibold text-[#030F35]">
                  Sign in to save this course
                </h2>
                <p className="text-sm text-gray-500 mt-1">
                  Create an account or sign in to bookmark courses and track
