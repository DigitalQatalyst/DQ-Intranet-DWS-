...
function validateSlugChange(
  ...
  if (isGHCGuide && GHC_SLUGS.has(newSlug)) {
    alert(
  ...
  return globalThis.confirm(
    `⚠️ Warning: You are changing the slug from "${currentSlug}" to "${newSlug}".\n\n` +
      `This will affect how the guide is accessed.\n\nAre you sure you want to continue?`,
  );
}

function logGHCUpdate(id: string, guide: Guide, payload: Guide): void {
  console.log(`[Admin Editor] Updating GHC guide: ${guide.slug} (ID: ${id})`);
  console.log(`[Admin Editor] Title: ${guide.title}`);
  console.log(
    `[Admin Editor] Body length: ${(payload as any).body?.length || 0} chars`,
  );
  if (payload.slug !== guide.slug) {
    console.error(
      `[Admin Editor] ERROR: Slug mismatch detected! Expected: ${guide.slug}, Got: ${payload.slug}`,
    );
  }
}
...
const GuideEditor: React.FC = () => {
  ...
  const [sharedContentWarning, setSharedContentWarning] = useState<
    string | null
  >(null);
    ...
    (async () => {
      try {
        ...
        if (id && g.slug && GHC_SLUGS.has(g.slug)) {
          checkForSharedContent(g.slug, (g as any).body || "");
        }
      } finally {
        setLoading(false);
      }
  ...
  async function checkForSharedContent(
    ...
    try {
      ...
      const sharedWith = allGHCGuides.filter(
        (g) =>
          g.slug !== currentSlug &&
          g.body &&
  ...
  const onChange = (k: keyof Guide, v: any) =>
    setGuide((prev) => ({ ...prev, [k]: v }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (typeof payload.contributors === "string")
      payload.contributors = parseList(payload.contributors as any);
    ...
    if (payload.slug && guide.slug && payload.slug !== guide.slug) {
      if (!validateSlugChange(guide.slug, payload.slug, isGHCGuide)) return;
    }

    if (isGHCGuide) {
      logGHCUpdate(id, guide, payload);
    } else {
      console.log(
        `[Admin Editor] Updating guide: id=${id}, slug=${guide.slug}, title=${guide.title}`,
      );
    }
    ...
    try {
      ...
      if (isGHCGuide)
        console.log(
          `[Admin Editor] ✅ Successfully updated GHC guide: ${guide.slug}`,
        );
          ...
          >
            <p
              className={`text-sm font-semibold ${
                GHC_SLUGS.has(guide.slug) ? "text-amber-900" : "text-blue-800"
              }`}
            >
              <strong>Editing:</strong>{" "}
              <code
                className={`px-2 py-1 rounded ${
                  GHC_SLUGS.has(guide.slug) ? "bg-amber-100" : "bg-blue-100"
                }`}
              >
                {guide.slug}
              </code>
            ...
            {GHC_SLUGS.has(guide.slug) && (
              <p className="text-xs text-amber-700 mt-1 font-medium">
                ⚠️ GHC Element: Slug is fixed and cannot be changed to another
                GHC slug
              </p>
            )}
            <p
              className={`text-xs mt-1 ${
                GHC_SLUGS.has(guide.slug) ? "text-amber-600" : "text-blue-600"
              }`}
            >
              Guide ID: {id}
            </p>
              ...
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  <p className="text-red-800 font-semibold mb-1">
                    CRITICAL: Shared Content Detected
                  </p>
      ...
      <form className="space-y-4 max-w-3xl" onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              type="date"
              className="border rounded px-3 py-2 w-full"
              value={(guide.lastUpdatedAt || "").slice(0, 10)}
              onChange={(e) => onChange("lastUpdatedAt", e.target.value)}
            />
          </div>
        </div>
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => onChange("guideType", e.target.value)}
            >
              <option value="">—</option>
              {tax.guideType.map((o) => (
          ...
          <div>
            ...
            <select
              className="border rounded px-3 py-2 w-full"
              value={guide.businessStage || ""}
              onChange={(e) => onChange("businessStage", e.target.value)}
            >
              <option value="">—</option>
              {tax.businessStage.map((o) => (
                <option key={o.id} value={o.name}>
                  {o.name}
                </option>
              ))}
        ...
        <div>
          ...
          <input
            className="border rounded px-3 py-2 w-full"
            value={
              Array.isArray(guide.contributors)
                ? guide.contributors.join(", ")
                : (guide.contributors as any) || ""
            }
            onChange={(e) => onChange("contributors", e.target.value)}
          />
        </div>
        <div>
          <label className="block text-sm font-medium">
          ...
          <textarea
            ...
            onChange={(e) => {
              try {
                onChange("steps", JSON.parse(e.target.value));
              } catch {
                onChange("steps", guide.steps);
              }
            }}
          />
        </div>
