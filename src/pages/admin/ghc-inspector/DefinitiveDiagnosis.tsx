...
function buildProofData(
  guides: Guide[],
  slugSet: Set<string>,
  duplicateSlugs: boolean,
  sharedBodies: [string, Guide[]][],
) {
  return {
...
function getDiagnosisMessage(
  ...
  if (duplicateSlugs) {
    return "❌ SUPABASE ISSUE: Duplicate slugs found in database. This is a data integrity problem.";
  }
  ...
  useEffect(() => {
    async function diagnose() {
      try {
        ...
        const bodyMap = buildBodyMap(guides as Guide[]);
        ...
        setDiagnosis(getDiagnosisMessage(duplicateSlugs, sharedBodies));
      } catch (err: any) {
    ...
    <div className="bg-white border-2 border-gray-300 rounded-lg p-6 mb-6">
        ...
        <div className="mt-4">
          ...
          <div className="bg-gray-50 p-4 rounded border border-gray-200">
            <div className="space-y-2 text-sm">
              <p>
                <strong>Total GHC guides in database:</strong>{" "}
                {proof.totalGuides}
              </p>
              <p>
                <strong>Unique slugs:</strong> {proof.uniqueSlugs}
              </p>
              <p>
                <strong>Duplicate slugs:</strong>{" "}
                {proof.duplicateSlugs ? "❌ YES" : "✅ NO"}
              </p>
              <p>
                <strong>Groups with shared body content:</strong>{" "}
                {proof.sharedBodyGroups}
              </p>

              {proof.sharedBodyGroups > 0 && (
                <div className="mt-4">
                  <p className="font-bold text-red-700 mb-2">
                    ⚠️ Shared Content Groups:
                  </p>
                  {proof.sharedBodyDetails.map((group: any, idx: number) => (
                    <div
                      key={idx}
                      className="mb-3 p-3 bg-red-100 border border-red-300 rounded"
                    >
                      <p className="font-semibold text-red-900 mb-1">
                        Group {idx + 1}: {group.guides.length} guide(s) with
                        identical content
                      </p>
                      ...
                      <p className="text-xs text-red-600 mt-1">
                        Body length: {group.bodyLength} chars | Preview:{" "}
                        {group.bodyPreview}...
                      </p>
                    </div>
                  ))}
                ...
                <div className="mt-2 space-y-1 text-xs font-mono bg-white p-3 rounded border">
                  {proof.allGuides.map((g: any) => (
                    <div key={g.id} className="border-b border-gray-200 pb-1">
                      <strong>{g.slug}</strong> (ID: {g.id}) | Body:{" "}
                      {g.bodyLength} chars | "{g.bodyPreview}..."
                    </div>
                  ))}
                </div>
              </details>
      ...
      <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded">
        <h3 className="font-bold text-blue-900 mb-2">
          How the UI Works (Verified):
        </h3>
        <ol className="list-decimal list-inside space-y-1 text-sm text-blue-800">
          <li>
            Each GHC page has a hardcoded slug (e.g.,{" "}
            <code className="bg-blue-100 px-1 rounded">'dq-vision'</code>)
          </li>
          <li>
            UI queries:{" "}
          ...
          <li>
            UI displays the{" "}
            <code className="bg-blue-100 px-1 rounded">body</code> content from
            that specific record
          </li>
        </ol>
        <p className="mt-2 text-sm text-blue-700 font-semibold">
          ✅ The UI code is correct. If multiple pages show the same content, it
          means multiple database records have identical{" "}
          <code className="bg-blue-100 px-1 rounded">body</code> content.
        </p>
      </div>
    </div>
  );
