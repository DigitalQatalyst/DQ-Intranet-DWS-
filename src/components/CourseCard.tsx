  ...
  return <div className="flex flex-col min-h-[340px] bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-200" onClick={onQuickView}>
    ...
    <div className="mt-auto border-t border-gray-100 p-4 pt-5">
      <div className="flex justify-between gap-2">
        <button onClick={handleViewDetails} className="px-4 py-2 text-xs font-medium text-blue-600 bg-white border border-blue-600 hover:bg-blue-50 transition-colors whitespace-nowrap min-w-[120px] flex-1">
          View Details
        </button>
        <button onClick={e => {
          e.stopPropagation();
          navigate(`/onboarding/${course.id}`);
        }} className="px-4 py-2 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 transition-colors whitespace-nowrap flex-1">
          Start Learning
        </button>
      </div>
    </div>
