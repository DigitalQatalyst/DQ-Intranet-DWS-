...
// Power Automate API endpoint — configured via VITE_POWER_AUTOMATE_TECH_SUPPORT_URL in .env
const POWER_AUTOMATE_API_URL = import.meta.env.VITE_POWER_AUTOMATE_TECH_SUPPORT_URL as string;
...
export function TechSupportForm({ isOpen, onClose }: TechSupportFormProps) {
  ...
  const submitTechSupportRequest = async (payload: ApiRequestPayload): Promise<void> => {
    if (!POWER_AUTOMATE_API_URL) {
      throw new Error('Tech support endpoint is not configured. Please contact your administrator.');
    }
