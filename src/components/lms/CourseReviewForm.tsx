...
export const CourseReviewForm: React.FC<CourseReviewFormProps> = ({
  ...
  const [starRating, setStarRating] = useState(
    existingReview?.star_rating || 0,
  );
  const [hoverRating, setHoverRating] = useState(0);
  const [keyLearning, setKeyLearning] = useState(
    existingReview?.key_learning || "",
  );
  const [engagingPart, setEngagingPart] = useState<EngagingPart | "">(
    existingReview?.engaging_part || "",
  );
  const [generalFeedback, setGeneralFeedback] = useState(
    existingReview?.general_feedback || "",
  );
  ...
  const isValid = isReviewFormValid(
    starRating,
    keyLearning,
    engagingPart,
    generalFeedback,
    ...
    return (
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl border border-green-100 p-8 text-center">
        ...
        <p className="text-sm text-gray-500">
          Your review for <span className="font-medium">{courseTitle}</span> has
          been {isEditMode ? "updated" : "submitted"}.
        </p>
      </div>
    );
  }
  ...
  const renderSubmitButton = () => {
    ...
    return (
      <button
        ...
        className={`ml-auto px-6 py-3 rounded-xl font-semibold text-white transition-all flex items-center gap-2 ${btnClass}`}
      >
        {isSubmitting ? (
          <>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-2">
            What is the #1 thing you learned that you plan to apply in your work
            this week? <span className="text-red-500">*</span>
          </label>
          <textarea
            value={keyLearning}
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-3">
            What was the most engaging part of the course?{" "}
