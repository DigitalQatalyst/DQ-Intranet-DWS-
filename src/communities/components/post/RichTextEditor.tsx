...
import { Bold, Italic, List, ListOrdered, Code, Quote, BoxIcon } from 'lucide-react';
import './prosemirror.css';

interface RichTextEditorProps {
  readonly content: string;
  readonly onUpdate: (html: string, text: string) => void;
  readonly placeholder?: string;
  readonly maxLength?: number;
  readonly mode?: 'short' | 'long';
  readonly onModeChange?: (mode: 'short' | 'long') => void;
}
...
function ToolbarButton({ onClick, isActive, icon: Icon, label }: ToolbarButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`px-2 py-1 rounded-md transition-all duration-200 ease-in-out text-sm ${isActive ? 'bg-brand-light-blue text-brand-blue' : 'text-gray-600 hover:text-brand-blue'}`}
      title={label}
    >
...
export function RichTextEditor({
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor: ed }) => {
      ...
      if (charCount > effectiveMax) return;
      onUpdate(html, text);
    }
  });
  ...
  return (
    <div className={`border border-gray-300 rounded-md bg-white focus-within:ring-2 ring-brand-teal transition-all duration-200 ${percentage >= 100 ? 'border-red-500' : ''} flex flex-col ${currentMode === 'long' ? 'max-h-[650px]' : 'max-h-[350px]'}`}>
      {/* Fixed Toolbar */}
      <div className="flex-shrink-0 bg-gray-100 border-b border-gray-200 px-3 py-2 flex gap-2 rounded-t-lg flex-wrap">
        ...
        <ToolbarButton onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} isActive={editor.isActive('heading', { level: 2 })} icon={BoxIcon} label="Heading" />
      ...
      <div className="flex-1 overflow-y-auto cursor-text">
        <EditorContent
          editor={editor}
          className={`max-w-none p-3 focus:outline-none ${currentMode === 'long' ? 'prose prose-lg min-h-[400px]' : 'prose prose-sm min-h-[200px]'} [&_.ProseMirror]:${currentMode === 'long' ? 'min-h-[400px]' : 'min-h-[200px]'} [&_.ProseMirror]:outline-none [&_.ProseMirror]:cursor-text`}
        />
      </div>
      ...
      <div className="px-3 py-2 text-xs border-t border-gray-200 bg-gray-50 flex items-center justify-between">
        ...
        {percentage >= 100 && (
          <span className="text-red-600 font-medium">Character limit reached</span>
        )}
      </div>
    </div>
  );
